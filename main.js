...
function init() {
    ...
    // *** BUG FIX: We now ONLY check our custom localStorage session ***
    const storedSession = localStorage.getItem('app_user_session');
    
    if (storedSession) {
        const userData = JSON.parse(storedSession);
        // Important: Check if the stored user is for the correct website grade
        if (userData.grade === currentGrade) {
            currentUser = userData;
            showMainScreen();
        } else {
            // User is on the wrong site (e.g., M1 user on M2 site), log them out.
            handleLogout();
        }
    } else {
        showLoginScreen();
...
async function handleLogin(event) {
    ...
    if (error || !users || users.length === 0) {
        loginError.textContent = 'ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง หรือคุณอาจอยู่ผิดเว็บชั้นเรียน';
        return;
    }
...
// ================================================================
// UI & DISPLAY LOGIC (No changes in this section)
// ================================================================
