...
serve(async (req) => {
  ...
  const corsHeaders = {
    ...
    'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, x-admin-auth', // อนุญาต Headers ที่จำเป็น รวมถึง X-Admin-Auth
  }
  ...
  console.log('X-Admin-Auth Header received (Base64 Token):', base64Token);

  if (!base64Token) { // ถ้าไม่มี Token
    console.error('Error: X-Admin-Auth token missing. Returning 401.');
    return new Response(JSON.stringify({ error: 'Authorization token missing. Please log in as Admin.' }), { // ปรับปรุงข้อความ
      headers: { 'Content-Type': 'application/json', ...corsHeaders },
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: 'Invalid token format. Please log in again.' }), { // ปรับปรุงข้อความ
      headers: { 'Content-Type': 'application/json', ...corsHeaders },
      status: 401, // Unauthorized
    })
  }
  ...
  const shouldBreak = Deno.env.get('SHOULD_BREAK_FOR_FOR_DEBUG'); // ใช้ SHOULD_BREAK_FOR_FOR_DEBUG เนื่องจากชื่อ Secret ที่คุณมีเป็น SHOULD_BREAK_FOR_DEBUG
